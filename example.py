...
class SRAPI():
    ...
    def _post(self,endpoint,post_data):
        data = {'api_key':self.api_key,'async':self.async,'data':post_data}
    ...
    def add_user(self,email,zipcode=None,user_id=None,data_dict={}):
        payload = [{"email":email,"user_id":user_id, "data_dict":data_dict}]
        encoded_payload=simplejson.dumps(payload)
        return self._post('users/',encoded_payload)

    def add_item(self,item_name,item_id,item_type='product',data_dict={},variants={}):
    ...
    def add_action(self,action_hook="purchases/",user_id=None,item_id=None,email=None,item_name=None,timestamp=None,price=None,zipcode=None):
        payload = [{'user_id':user_id,'item_id':item_id,'price':price,'email':email,'item_name':item_name,'timestamp':timestamp,'zipcode':zipcode}]
        encoded_payload=simplejson.dumps(payload)
    ...
    def get_predictions(self,email,market=None,starts=None,expires=None):
        return self._get('items/',{'email':email,'market':market,'starts':starts,'expires':expires})
